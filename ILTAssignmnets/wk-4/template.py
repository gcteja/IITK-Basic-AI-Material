...
--------------------------------------------------------------------------
below 9 examples I tried. Except last two, rest all worked fine. 
-------------------------------------------------------------------------------------------
Terminal Input: python template.py "What color is a ripe banana?" "Red" "Blue" "Yellow" "Green"
Output: C

Terminal Input: python template.py "Which planet is the largest?" "Mars" "Jupiter" "Saturn" "Earth"
Output: B

Terminal Input: python template.py "What is the capital of France?" "Paris" "London" "Berlin" "Madrid"
Output: A

Terminal Input: python template.py "Which element has the chemical symbol O?" "Gold" "Oxygen" "Silver" "Iron"
Output: B

Terminal Input: python template.py "Which is the biggest retailer chain of stores in USA?" "Costco" "Walmart" "Massmart" "D-Mart"
Output: B

Terminal Input: python template.py "Which is the biggest retailer chain of stores in USA?" "None of These" "Patel" "Flipkart" "ASDA"
Output: A

Terminal Input: python template.py "what is result of 2x3?" "2" "6" "8" "1"
Output: B

Terminal Input: python template.py "what is result of 3+9?" "12" "27" "9" "0"
Output: B --> which is wrong. It is taking multiplication only even I mention addition. May be limitation of the Flan-T5 modal

Terminal Input: python template.py "Who is The Current President of India?" "D Murmu" "APJ Kalam" "Manmohan Singh" "Modi"
Output: was giving C or D. --> May be LLMs have a specific date when their training ended. Flan-T5 may not have the most current information about the presidency in its internal "memory"
-----------------------------------------------------------------------------------------------------------------------------------------
ALL THE BEST!!
"""
...
def llm_function(model,tokenizer,question,option_a,option_b,option_c,option_d):
    ...
    # We use a clear MCQ format to guide the Flan-T5-XL model
    prompt = (
        f"Question: {question}\n"
        f"Options:\n(A) {option_a}\n(B) {option_b}\n(C) {option_c}\n(D) {option_d}\n"
    ...
    # 3. Pass to model (Flan-T5-XL) to get deterministic output
    output_tokens = model.generate(**inputs, max_new_tokens=2)
    
    # 4 & 5. Extract and Clean output
    raw_output = tokenizer.decode(output_tokens[0], skip_special_tokens=True).strip().upper()
    
    # 6. Final cleaning to ensure we return ONLY A, B, C, or D. 
    # Since it must provide one of the four options (A/B/C/D), defaulting to "A" to avoid empty string or any error message in model failure cases.
